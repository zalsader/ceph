...
// Global Daos indices
#define RGW_DAOS_USERS_IDX_NAME       "motr.rgw.users"
#define RGW_DAOS_BUCKET_INST_IDX_NAME "motr.rgw.bucket.instances"
#define RGW_DAOS_BUCKET_HD_IDX_NAME   "motr.rgw.bucket.headers"
//#define RGW_DAOS_BUCKET_ACL_IDX_NAME  "motr.rgw.bucket.acls"

// A simplified metadata cache implementation.
// Note: DaosObjMetaCache doesn't handle the IO operations to Daos. A proxy
...
protected:
  ...
  //
  // Beaware: Daos object data is not cached in current POC as RGW!
...
public:
  ...
  // It is feasible to implement the functionality using Daos's FDMI after discussing
  // with Hua.
  ...
  DaosMetaCache(const DoutPrefixProvider *dpp, CephContext *cct) {
    cache.set_ctx(cct);
  }
};
...
protected:
  ...
  public:
    DaosNotification(Object* _obj, rgw::notify::EventType _type) : Notification(_obj, _type), obj(_obj), event_type(_type) {}
...
class DaosBucket : public Bucket {
  ...
  public:
    DaosBucket(DaosStore *_st)
      : store(_st),
      acls() {
      }

    DaosBucket(DaosStore *_st, User* _u)
      : Bucket(_u),
      store(_st),
      acls() {
      }
...
class DaosObject : public Object {
  private:
    ...
    // TODO: do it in another class? DaosPartObject : public DaosObject
    uint64_t part_off;
    uint64_t part_size;
    uint64_t part_num;
  ...
  private:
    //int read_attrs(const DoutPrefixProvider* dpp, Daos::Object::Read &read_op, optional_yield y, rgw_obj* target_obj = nullptr);
...
// TODO: implement it using Daos object locks.
class MPDaosSerializer : public MPSerializer {

  public:
    MPDaosSerializer(const DoutPrefixProvider *dpp, DaosStore* store, DaosObject* obj, const std::string& lock_name) {}
...
class DaosAtomicWriter : public Writer {
  ...
  public:
  DaosAtomicWriter(const DoutPrefixProvider *dpp,
          optional_yield y,
...
public:
  DaosMultipartWriter(const DoutPrefixProvider *dpp,
...
// cortx-s3server's design. Parts are stored in separate Daos objects.
// s3server uses a few auxiliary Daos indices to manage multipart
...
// DaosMultipartUpload::init(). (3) Extended metadata index
...
// considerations (how to handle Daos service crash when uploading an object)
// and to avoid to create too many Daos indices (I am not sure I understand
// why many Daos indices is bad.). In our POC, to keep it simple, only 2
...
public:
  ...
  DaosMultipartPart(RGWUploadPartInfo _info, DaosObject::Meta _meta) :
...
public:
  DaosMultipartUpload(DaosStore* _store, Bucket* _bucket, const std::string& oid,
...
class DaosStore : public Store {
  private:
    ...
    DaosMetaCache* obj_meta_cache;
    DaosMetaCache* user_cache;
    DaosMetaCache* bucket_inst_cache;
  ...
  public:
    ...
    DaosMetaCache* get_obj_meta_cache() {return obj_meta_cache;}
    DaosMetaCache* get_user_cache() {return user_cache;}
    DaosMetaCache* get_bucket_inst_cache() {return bucket_inst_cache;}
};
