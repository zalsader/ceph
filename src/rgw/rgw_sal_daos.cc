...
static std::string motr_global_indices[] = {
  RGW_DAOS_USERS_IDX_NAME,
  RGW_DAOS_BUCKET_INST_IDX_NAME,
  RGW_DAOS_BUCKET_HD_IDX_NAME
};

void DaosMetaCache::invalid(const DoutPrefixProvider *dpp,
                           const string& name)
{
  cache.invalidate_remove(dpp, name);
}

int DaosMetaCache::put(const DoutPrefixProvider *dpp,
                       const string& name,
...
int DaosMetaCache::get(const DoutPrefixProvider *dpp,
                       const string& name,
                       bufferlist& data)
{
  ObjectCacheInfo info;
...
int DaosMetaCache::remove(const DoutPrefixProvider *dpp,
                          const string& name)
...
int DaosMetaCache::distribute_cache(const DoutPrefixProvider *dpp,
...
int DaosMetaCache::watch_cb(const DoutPrefixProvider *dpp,
                            uint64_t notify_id,
                            uint64_t cookie,
...
void DaosMetaCache::set_enabled(bool status)
{
...
int DaosUser::create_bucket(const DoutPrefixProvider* dpp,
                            const rgw_bucket& b,
...
int DaosUser::read_stats(const DoutPrefixProvider *dpp,
...
int DaosUser::read_stats_async(const DoutPrefixProvider *dpp, RGWGetUserStats_CB *cb)
{
  return 0;
}

int DaosUser::complete_flush_stats(const DoutPrefixProvider *dpp, optional_yield y)
{
  return 0;
}

int DaosUser::read_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch, uint64_t end_epoch, uint32_t max_entries,
...
int DaosUser::trim_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch, uint64_t end_epoch)
{
  return 0;
}
...
int DaosUser::load_user(const DoutPrefixProvider *dpp,
                        optional_yield y)
{
...
int DaosUser::create_user_info_idx()
{
  string user_info_iname = "motr.rgw.user.info." + info.user_id.id;
  return store->create_motr_idx_by_name(user_info_iname);
}

int DaosUser::store_user(const DoutPrefixProvider* dpp,
                         optional_yield y, bool exclusive, RGWUserInfo* old_info)
{
  ...
  muinfo.encode(bl);
  rc = store->do_idx_op_by_name(RGW_DAOS_USERS_IDX_NAME,
...
int DaosBucket::remove_bucket(const DoutPrefixProvider *dpp, bool delete_children, bool forward_to_master, req_info* req_info, optional_yield y)
{
  int ret;
...
int DaosBucket::remove_bucket_bypass_gc(int concurrent_max, bool
...
int DaosBucket::put_info(const DoutPrefixProvider *dpp, bool exclusive, ceph::real_time _mtime)
{
  bufferlist bl;
  ...
  int rc = store->do_idx_op_by_name(RGW_DAOS_BUCKET_INST_IDX_NAME,
...
int DaosBucket::load_bucket(const DoutPrefixProvider *dpp, optional_yield y)
{
  ...
  bufferlist& blr = bl;
  auto iter =blr.cbegin();
  mbinfo.decode(iter); //Decode into DaosBucketInfo.
...
int DaosBucket::link_user(const DoutPrefixProvider* dpp, User* new_user, optional_yield y)
{
  bufferlist bl;
...
int DaosBucket::unlink_user(const DoutPrefixProvider* dpp, User* new_user, optional_yield y)
{
...
int DaosBucket::create_multipart_indices()
{
  ...
  // DaosMultipartUpload::init()) and will be removed when the upload
  // is completed (DaosMultipartUpload::complete()).
  // DaosBucket::list_multiparts() will scan this index to return all
...
int DaosBucket::read_stats_async(const DoutPrefixProvider *dpp, int shard_id, RGWGetBucketStats_CB *ctx)
{
  return 0;
}

int DaosBucket::sync_user_stats(const DoutPrefixProvider *dpp, optional_yield y)
{
  return 0;
}
...
int DaosBucket::chown(const DoutPrefixProvider *dpp, User* new_user, User* old_user, optional_yield y, const std::string* marker)
{
...
int DaosBucket::check_quota(const DoutPrefixProvider *dpp, RGWQuotaInfo& user_quota, RGWQuotaInfo& bucket_quota, uint64_t obj_size,
...
int DaosBucket::merge_and_store_attrs(const DoutPrefixProvider *dpp, Attrs& new_attrs, optional_yield y)
{
...
int DaosBucket::try_refresh_info(const DoutPrefixProvider *dpp, ceph::real_time *pmtime)
{
  return 0;
}

/* XXX: usage and stats not supported in the first pass */
int DaosBucket::read_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch, uint64_t end_epoch,
...
int DaosBucket::trim_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch, uint64_t end_epoch)
{
  return 0;
}

int DaosBucket::remove_objs_from_index(const DoutPrefixProvider *dpp, std::list<rgw_obj_index_key>& objs_to_unlink)
{
...
int DaosBucket::check_index(const DoutPrefixProvider *dpp, std::map<RGWObjCategory, RGWStorageStats>& existing_stats, std::map<RGWObjCategory, RGWStorageStats>& calculated_stats)
{
...
int DaosBucket::set_tag_timeout(const DoutPrefixProvider *dpp, uint64_t timeout)
{
...
int DaosBucket::set_acl(const DoutPrefixProvider *dpp, RGWAccessControlPolicy &acl, optional_yield y)
{
  int ret = 0;
  bufferlist aclbl;
...
int DaosBucket::list(const DoutPrefixProvider *dpp, ListParams& params, int max, ListResults& results, optional_yield y)
{
  int rc;
  vector<string> keys(max);
  vector<bufferlist> vals(max);
...
int DaosBucket::list_multiparts(const DoutPrefixProvider *dpp,
      const string& prefix,
      string& marker,
      const string& delim,
...
int DaosBucket::abort_multiparts(const DoutPrefixProvider *dpp, CephContext *cct)
{
  return 0;
}
...
const RGWZoneGroup& DaosZone::get_zonegroup()
{
  return *zonegroup;
}
...
const RGWZoneParams& DaosZone::get_params()
{
  return *zone_params;
}
...
const RGWRealm& DaosZone::get_realm()
{
  return *realm;
}
...
bool DaosZone::is_writeable()
{
  return true;
}

bool DaosZone::get_redirect_endpoint(std::string* endpoint)
{
  return false;
}
...
int DaosObject::get_obj_state(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWObjState **_state, optional_yield y, bool follow_olh)
{
  if (state == nullptr)
...
//  int DaosObject::read_attrs(const DoutPrefixProvider* dpp, Daos::Object::Read &read_op, optional_yield y, rgw_obj* target_obj)
//  {
...
int DaosObject::set_obj_attrs(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, Attrs* setattrs, Attrs* delattrs, optional_yield y, rgw_obj* target_obj)
{
  ldpp_dout(dpp, 20) << "DEBUG: DaosObject::set_obj_attrs()" << dendl;
  return 0;
}

int DaosObject::get_obj_attrs(RGWObjectCtx* rctx, optional_yield y, const DoutPrefixProvider* dpp, rgw_obj* target_obj)
{
  ...
  ldpp_dout(dpp, 20) << "DaosObject::get_obj_attrs(): "
                    << bname << "/" << key << dendl;
...
int DaosObject::modify_obj_attrs(RGWObjectCtx* rctx, const char* attr_name, bufferlist& attr_val, optional_yield y, const DoutPrefixProvider* dpp)
{
...
int DaosObject::delete_obj_attrs(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, const char* attr_name, optional_yield y)
{
  rgw_obj target = get_obj();
  Attrs rmattr;
  bufferlist bl;
...
void DaosObject::set_atomic(RGWObjectCtx* rctx) const
{
  return;
}
...
/* RGWObjectCtx will be moved out of sal */
/* XXX: Placeholder. Should not be needed later after Dan's patch */
void DaosObject::set_prefetch_data(RGWObjectCtx* rctx)
{
  return;
}
...
/* RGWObjectCtx will be moved out of sal */
/* XXX: Placeholder. Should not be needed later after Dan's patch */
void DaosObject::set_compressed(RGWObjectCtx* rctx)
{
  return;
}
...
int DaosObject::omap_get_vals(const DoutPrefixProvider *dpp, const std::string& marker, uint64_t count,
...
int DaosObject::omap_get_all(const DoutPrefixProvider *dpp, std::map<std::string, bufferlist> *m,
    optional_yield y)
{
  return 0;
}

int DaosObject::omap_get_vals_by_keys(const DoutPrefixProvider *dpp, const std::string& oid,
...
int DaosObject::omap_set_val_by_key(const DoutPrefixProvider *dpp, const std::string& key, bufferlist& val,
    bool must_exist, optional_yield y)
{
  return 0;
}

MPSerializer* DaosObject::get_serializer(const DoutPrefixProvider *dpp, const std::string& lock_name)
{
  return new MPDaosSerializer(dpp, store, this, lock_name);
}

int DaosObject::transition(RGWObjectCtx& rctx,
    Bucket* bucket,
...
bool DaosObject::placement_rules_match(rgw_placement_rule& r1, rgw_placement_rule& r2)
{
  /* XXX: support single default zone and zonegroup for now */
  return true;
}

int DaosObject::dump_obj_layout(const DoutPrefixProvider *dpp, optional_yield y, Formatter* f, RGWObjectCtx* obj_ctx)
{
  return 0;
}
...
int DaosObject::DaosReadOp::prepare(optional_yield y, const DoutPrefixProvider* dpp)
{
  int rc;
...
int DaosObject::DaosReadOp::read(int64_t off, int64_t end, bufferlist& bl, optional_yield y, const DoutPrefixProvider* dpp)
{
...
int DaosObject::DaosReadOp::iterate(const DoutPrefixProvider* dpp, int64_t off, int64_t end, RGWGetDataCB* cb, optional_yield y)
{
  int rc;
...
int DaosObject::DaosReadOp::get_attr(const DoutPrefixProvider* dpp, const char* name, bufferlist& dest, optional_yield y)
{
  //return 0;
  return -ENODATA;
}
...
int DaosObject::DaosDeleteOp::delete_obj(const DoutPrefixProvider* dpp, optional_yield y)
{
  ...
  if (rc < 0) {
    ldpp_dout(dpp, 0) << "Failed to delete the object from Daos. " << dendl;
    return rc;
  }
...
int DaosObject::delete_object(const DoutPrefixProvider* dpp, RGWObjectCtx* obj_ctx, optional_yield y, bool prevent_versioning)
{
...
int DaosObject::delete_obj_aio(const DoutPrefixProvider* dpp, RGWObjState* astate,
...
int DaosObject::copy_object(RGWObjectCtx& obj_ctx,
    User* user,
    req_info* info,
...
int DaosObject::swift_versioning_restore(RGWObjectCtx* obj_ctx,
    bool& restored,
    const DoutPrefixProvider* dpp)
{
  return 0;
...
DaosAtomicWriter::DaosAtomicWriter(const DoutPrefixProvider *dpp,
          optional_yield y,
...
void DaosObject::obj_name_to_motr_fid(struct m0_uint128 *obj_fid)
{
...
int DaosObject::create_mobj(const DoutPrefixProvider *dpp, uint64_t sz)
{
  if (mobj != nullptr) {
...
int DaosObject::open_mobj(const DoutPrefixProvider *dpp)
{
  this->obj_name_to_motr_fid(&fid);
...
int DaosObject::delete_mobj(const DoutPrefixProvider *dpp)
{
  int rc;
...
int DaosObject::write_mobj(const DoutPrefixProvider *dpp, bufferlist&& data, uint64_t offset)
{
  int rc;
  unsigned bs, left;
  struct m0_op *op;
...
int DaosObject::read_mobj(const DoutPrefixProvider* dpp, int64_t off, int64_t end, RGWGetDataCB* cb)
{
  int rc;
  unsigned bs, actual;
  struct m0_op *op;
  ...
  ldpp_dout(dpp, 20) << "DaosObject::read_mobj(): off=" << off <<
                       " end=" << end << dendl;
  ...
  // across parity group boundary. And Daos only allows page-size aligned
  // offset.
  //
  ...
  ldpp_dout(dpp, 20) << "DaosObject::read_mobj(): bs=" << bs << dendl;
  ...
  for (; off < end; off += actual) {
    ...
    ldpp_dout(dpp, 20) << "DaosObject::read_mobj(): init read op rc=" << rc << dendl;
    if (rc != 0)
      goto out;
    m0_op_launch(&op, 1);
    rc = m0_op_wait(op, M0_BITS(M0_OS_FAILED, M0_OS_STABLE), M0_TIME_NEVER) ?:
    ...
    ldpp_dout(dpp, 20) << "DaosObject::read_mobj(): call cb to process data" << dendl;
...
int DaosObject::get_bucket_dir_ent(const DoutPrefixProvider *dpp, rgw_bucket_dir_entry& ent)
{
  int rc = 0;
  string bucket_index_iname = "motr.rgw.bucket.index." + this->get_bucket()->get_name();
  int max = 1000;
...
int DaosObject::update_version_entries(const DoutPrefixProvider *dpp)
{
  int rc;
  int max = 10;
  vector<string> keys(max);
...
int DaosObject::get_part_objs(const DoutPrefixProvider* dpp,
                              std::map<int, std::unique_ptr<DaosObject>>& part_objs)
{
  int rc;
  int total_parts = 0;
  int max_parts = 1000;
  ...
  do {
    ...
    for (auto part_iter = parts.begin(); part_iter != parts.end(); ++part_iter) {
      ...
      DaosMultipartPart *mmpart = static_cast<DaosMultipartPart *>(mpart);
      ...
      std::unique_ptr<rgw::sal::DaosObject> mobj(static_cast<rgw::sal::DaosObject *>(obj.release()));
...
int DaosObject::open_part_objs(const DoutPrefixProvider* dpp,
                               std::map<int, std::unique_ptr<DaosObject>>& part_objs)
{
  //for (auto& iter: part_objs) {
  for (auto iter = part_objs.begin(); iter != part_objs.end(); ++iter) {
    DaosObject* obj = static_cast<DaosObject *>(iter->second.get());
...
int DaosObject::delete_part_objs(const DoutPrefixProvider* dpp)
{
  std::unique_ptr<rgw::sal::MultipartUpload> upload;
  upload = this->get_bucket()->get_multipart_upload(this->get_name(), string());
  std::unique_ptr<rgw::sal::DaosMultipartUpload> mupload(static_cast<rgw::sal::DaosMultipartUpload *>(upload.release()));
  return mupload->delete_parts(dpp);
}

int DaosObject::read_multipart_obj(const DoutPrefixProvider* dpp,
...
unsigned DaosObject::get_optimal_bs(unsigned len)
{
...
unsigned DaosAtomicWriter::populate_bvec(unsigned len, bufferlist::iterator &bi)
{
  unsigned i, l, done = 0;
  const char *data;
...
int DaosAtomicWriter::complete(size_t accounted_size, const std::string& etag,
			      ...
			      rgw_placement_rule& dest_placement, rgw::sal::Attrs& attrs)
{
  ...
  string obj_part_iname = "motr.rgw.object." + bucket->get_name() + "." + oid + ".parts";
  ldpp_dout(dpp, 20) << "DaosMultipartUpload::init(): object part index=" << obj_part_iname << dendl;
...
int DaosMultipartUpload::list_parts(const DoutPrefixProvider *dpp, CephContext *cct,
				     ...
				     bool assume_unsorted)
{
  ...
  std::string oid = mp_obj.get_key();
  string obj_part_iname = "motr.rgw.object." + bucket->get_name() + "." + oid + ".parts";
  ldpp_dout(dpp, 20) << "DaosMultipartUpload::list_parts(): object part index = " << obj_part_iname << dendl;
  key_vec[0].clear();
  key_vec[0] = "part.";
  char buf[32];
  ...
  ldpp_dout(dpp, 20) << "DaosMultipartUpload::list_parts(): marker = " << marker << dendl;
  for (const auto& bl: val_vec) {
    if (bl.length() == 0)
      break;

    ldpp_dout(dpp, 20) << "DaosMultipartUpload::list_parts(): get part_info "  << dendl;
    ...
    ldpp_dout(dpp, 20) << "DaosMultipartUpload::list_parts(): part_num=" << info.num
    ...
    if ((int)info.num > marker) {
      last_num = info.num;
      parts.emplace(info.num, std::make_unique<DaosMultipartPart>(info, meta));
    }
  ...
  ldpp_dout(dpp, 20) << "DaosMultipartUpload::list_parts(): truncated=" << *truncated << dendl;
...
int DaosMultipartUpload::complete(const DoutPrefixProvider *dpp,
				   ...
				   RGWObjectCtx* obj_ctx)
{
  ...
  do {
    ldpp_dout(dpp, 20) << "DaosMultipartUpload::complete(): list_parts()" << dendl;
    ...
    ldpp_dout(dpp, 20) << "DaosMultipartUpload::complete(): parts.size()=" << parts.size() << dendl;
    ...
    for (auto obj_iter = parts.begin();
	 ...
	 ++etags_iter, ++obj_iter, ++handled_parts) {
      ...
      ldpp_dout(dpp, 20) << "DaosMultipartUpload::complete(): part_size=" << part_size << dendl;
      ...
      ldpp_dout(dpp, 20) << "DaosMultipartUpload::complete(): calc etag " << dendl;

      string oid = mp_obj.get_part(part->num);
      rgw_obj src_obj;
      src_obj.init_ns(bucket->get_key(), oid, mp_ns);

#if 0 // does Daos backend need it?
      ...
      ldpp_dout(dpp, 0) << "DaosMultipartUpload::complete(): manifest " << dendl;
#endif
      ...
      ldpp_dout(dpp, 20) << "DaosMultipartUpload::complete(): part compression" << dendl;
      if (part_compressed) {
      ...
      ldpp_dout(dpp, 20) << "DaosMultipartUpload::complete(): off=" << off << ", accounted_size = " << accounted_size << dendl;
    }
  } while (truncated);
  ...
  ldpp_dout(dpp, 20) << "DaosMultipartUpload::complete(): read entry from bucket multipart index rc=" << rc << dendl;
  if (rc < 0)
    return rc;
  rgw_bucket_dir_entry ent;
  bufferlist& blr = bl;
  ...
  ldpp_dout(dpp, 20) << "DaosMultipartUpload::complete(): remove from bucket multipartindex " << dendl;
  return store->do_idx_op_by_name(bucket_multipart_iname,
                                  M0_IC_DEL, meta_obj->get_key().to_str(), bl);
}

int DaosMultipartUpload::get_info(const DoutPrefixProvider *dpp, optional_yield y, RGWObjectCtx* obj_ctx, rgw_placement_rule** rule, rgw::sal::Attrs* attrs)
{
  if (!rule && !attrs) {
    return 0;
  }
...
{
  return std::make_unique<DaosMultipartWriter>(dpp, y, this,
...
int DaosMultipartWriter::prepare(optional_yield y)
{
  ...
  part_obj = std::make_unique<DaosObject>(this->store, rgw_obj_key(part_obj_name), head_obj->get_bucket());
  if (part_obj == nullptr)
    return -ENOMEM;
...
int DaosMultipartWriter::complete(size_t accounted_size, const std::string& etag,
                       ceph::real_time *mtime, ceph::real_time set_mtime,
                       std::map<std::string, bufferlist>& attrs,
                       ceph::real_time delete_at,
                       const char *if_match, const char *if_nomatch,
                       ...
                       optional_yield y)
{
  ...
  ldpp_dout(dpp, 20) << "DaosMultipartWriter::complete(): object part index = " << obj_part_iname << dendl;
...
std::unique_ptr<RGWRole> DaosStore::get_role(std::string name,
    std::string tenant,
    std::string path,
...
int DaosStore::get_roles(const DoutPrefixProvider *dpp,
    optional_yield y,
...
std::unique_ptr<MultipartUpload> DaosBucket::get_multipart_upload(const std::string& oid,
                                std::optional<std::string> upload_id,
                                ACLOwner owner, ceph::real_time mtime)
{
  return std::make_unique<DaosMultipartUpload>(store, this, oid, upload_id, owner, mtime);
}

std::unique_ptr<Writer> DaosStore::get_append_writer(const DoutPrefixProvider *dpp,
        optional_yield y,
        std::unique_ptr<rgw::sal::Object> _head_obj,
        const rgw_user& owner, RGWObjectCtx& obj_ctx,
        const rgw_placement_rule *ptail_placement_rule,
...
std::unique_ptr<Writer> DaosStore::get_atomic_writer(const DoutPrefixProvider *dpp,
        optional_yield y,
        std::unique_ptr<rgw::sal::Object> _head_obj,
        const rgw_user& owner, RGWObjectCtx& obj_ctx,
        const rgw_placement_rule *ptail_placement_rule,
        uint64_t olh_epoch,
        const std::string& unique_tag) {
  return std::make_unique<DaosAtomicWriter>(dpp, y,
...
int DaosStore::get_user_by_access_key(const DoutPrefixProvider *dpp, const std::string& key, optional_yield y, std::unique_ptr<User>* user)
{
  RGWUserInfo uinfo;
  User *u;
  RGWObjVersionTracker objv_tracker;
...
int DaosStore::get_user_by_email(const DoutPrefixProvider *dpp, const std::string& email, optional_yield y, std::unique_ptr<User>* user)
{
  return 0;
}

int DaosStore::get_user_by_swift(const DoutPrefixProvider *dpp, const std::string& user_str, optional_yield y, std::unique_ptr<User>* user)
{
...
int DaosStore::get_bucket(const DoutPrefixProvider *dpp, User* u, const rgw_bucket& b, std::unique_ptr<Bucket>* bucket, optional_yield y)
{
  int ret;
  Bucket* bp;
...
int DaosStore::get_bucket(User* u, const RGWBucketInfo& i, std::unique_ptr<Bucket>* bucket)
{
  Bucket* bp;
...
int DaosStore::get_bucket(const DoutPrefixProvider *dpp, User* u, const std::string& tenant, const std::string& name, std::unique_ptr<Bucket>* bucket, optional_yield y)
{
  rgw_bucket b;
...
bool DaosStore::is_meta_master()
{
  return true;
}

int DaosStore::forward_request_to_master(const DoutPrefixProvider *dpp, User* user, obj_version *objv,
    bufferlist& in_data,
...
std::string DaosStore::zone_unique_id(uint64_t unique_num)
{
  return "";
}
...
std::unique_ptr<Notification> DaosStore::get_notification(rgw::sal::Object* obj,
    struct req_state* s,
...
int DaosStore::log_usage(const DoutPrefixProvider *dpp, map<rgw_user_bucket, RGWUsageBatch>& usage_info)
{
  return 0;
}

int DaosStore::log_op(const DoutPrefixProvider *dpp, string& oid, bufferlist& bl)
{
  return 0;
}

int DaosStore::register_to_service_map(const DoutPrefixProvider *dpp, const string& daemon_type,
    const map<string, string>& meta)
{
  return 0;
}

void DaosStore::get_quota(RGWQuotaInfo& bucket_quota, RGWQuotaInfo& user_quota)
{
  // XXX: Not handled for the first pass
  return;
}

int DaosStore::set_buckets_enabled(const DoutPrefixProvider *dpp, vector<rgw_bucket>& buckets, bool enabled)
{
  return 0;
}
...
RGWDataSyncStatusManager* DaosStore::get_data_sync_manager(const rgw_zone_id& source_zone)
{
  return 0;
}

int DaosStore::read_all_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch, uint64_t end_epoch,
    uint32_t max_entries, bool *is_truncated,
    RGWUsageIter& usage_iter,
    map<rgw_user_bucket, rgw_usage_log_entry>& usage)
{
...
int DaosStore::trim_all_usage(const DoutPrefixProvider *dpp, uint64_t start_epoch, uint64_t end_epoch)
{
  return 0;
}
...
int DaosStore::meta_list_keys_init(const DoutPrefixProvider *dpp, const string& section, const string& marker, void** phandle)
{
  return 0;
}

int DaosStore::meta_list_keys_next(const DoutPrefixProvider *dpp, void* handle, int max, list<string>& keys, bool* truncated)
{
  return 0;
}

void DaosStore::meta_list_keys_complete(void* handle)
{
  return;
}

std::string DaosStore::meta_get_marker(void* handle)
{
  return "";
}

int DaosStore::meta_remove(const DoutPrefixProvider *dpp, string& metadata_key, optional_yield y)
{
  return 0;
}

int DaosStore::open_idx(struct m0_uint128 *id, bool create, struct m0_idx *idx)
{
...
int DaosStore::do_idx_op(struct m0_idx *idx, enum m0_idx_opcode opcode,
...
int DaosStore::do_idx_next_op(struct m0_idx *idx,
...
int DaosStore::next_query_by_name(string idx_name,
...
int DaosStore::delete_motr_idx_by_name(string iname)
{
  struct m0_idx idx;
  struct m0_uint128 idx_id;
  struct m0_op *op = nullptr;
...
int DaosStore::open_motr_idx(struct m0_uint128 *id, struct m0_idx *idx)
{
  m0_idx_init(idx, &container.co_realm, id);
  return 0;
}
...
void DaosStore::index_name_to_motr_fid(string iname, struct m0_uint128 *id)
{
...
int DaosStore::do_idx_op_by_name(string idx_name, enum m0_idx_opcode opcode,
...
int DaosStore::create_motr_idx_by_name(string iname)
{
  struct m0_idx idx = {};
...
std::string DaosStore::get_cluster_id(const DoutPrefixProvider* dpp,  optional_yield y)
{
...
int DaosStore::init_metadata_cache(const DoutPrefixProvider *dpp,
                                   CephContext *cct)
{
  this->obj_meta_cache = new DaosMetaCache(dpp, cct);
  this->get_obj_meta_cache()->set_enabled(true);

  this->user_cache = new DaosMetaCache(dpp, cct);
  this->get_user_cache()->set_enabled(true);

  this->bucket_inst_cache = new DaosMetaCache(dpp, cct);
